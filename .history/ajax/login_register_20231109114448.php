...
if(isset($_POST['forgot_pass']))
{
    ...
    $u_exist = select("SELECT * FROM `user_ced` WHERE `email`=? LIMIT 1", [$data['email']],"s");
    ...
    else
    {
        ...
        if($u_fetch['is_verified']==0){
            ...
            else
            {
                ...
                $query = mysqli_query($con,"UPDATE `user_ced` SET `token`='$token', `t_expire`='$date'
                    WHERE `id`='$u_fetch[id]'");
...
if(isset($_POST['recover_user']))
{
    ...
    $query = "UPDATE `user_ced` SET `password`=? `token`=?, `t_expire`=?
        WHERE `email`=? and `token`=?";

    $values = [$enc_pass,null,null,$data['email'],$data[]]
}
?>