...
if(isset($_POST['toggle_status']))
{
    ...
    $q = "UPDATE `user_ced` SET `status`=? WHERE `id`=?";
...
if(isset($_POST['remove_user']))       
{
    ...
    $res = delete("DELETE FROM `user_ced` WHERE `id`=? AND `is_verified`=?",[$frm_data['user_id'],0],'ii');
...
{
    $frm_data = filtaretion($_POST);

    $query = "SELECT * FROM `user_ced` WHERE `name` LIKE ?";

    $res = select($query,[%$frm_data%[name]]);
    $i=1;
    $path = USERS_IMG_PATH;
